...
import { getInternationalMarketingJoinCta } from "@/data/pages/international-marketing";
import { getPartnerProgramSection } from "@/lib/i18n-shared";
import { useLanguage } from "@/context/LanguageContext";

export default function JoinSubscriptionCTA() {
  const { language } = useLanguage();
  const data = getInternationalMarketingJoinCta(language);
      ...
      <div className="relative z-10 mx-auto max-w-6xl px-4 lg:px-6">
        <div className="rounded-3xl border border-brand-gold/30 bg-white/[0.03] p-8 md:p-14 backdrop-blur-md shadow-2xl">
          ...
          <div className="mt-10 flex flex-wrap items-center justify-center gap-4">
            ...
            <Link
              href="/contact-us"
              className="inline-flex items-center justify-center rounded-full border border-white/20 bg-white/5 px-7 py-3.5 text-sm font-semibold text-white backdrop-blur-sm transition-all hover:border-brand-gold/60 hover:bg-white/10"
            >
              Contact Us
            </Link>
          </div>
